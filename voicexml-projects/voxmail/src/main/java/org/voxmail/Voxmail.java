...
public class Voxmail {
    ...
    private Voxmail() {
        this.voxmailDao = new VoxmailDAO();
        this.voxmailService = new VoxmailService();
        this.vmsm = new VoxmailSessionManagement();
    ...
    public VoxmailService getVoxmailService() {
        return this.voxmailService;
    }

    /**
     * Saves any changes made to the Voxmail model objects to the database. Should be called whenever the transaction is
     * complete. This is normally done in the services (Business Delegates), and generally should not be done in the
     * Action classes or the DAO classes.
     * 
    ...
    /**
     * This will cancel any changes made to the Voxmail model. At this point all references to any Voxmail objects
     * should be thrown away, and re-grabbed by going to a Service or DAO.
     * 
     * @throws VoxmailException
     */
    ...
    /**
     * Saves an individual object. This should not be commonly used, but only used when you want to do a partial
     * transaction. Normally Voxmail.commit() will save all the model changes.
     * 
     * @param object
     * @throws VoxmailException
     */
    public void saveObject(final Object object) throws VoxmailException {
        this.vmsm.saveObject(object);
    }

    /**
     * This will clean up any thread context state, this should be called when the thread is done doing Voxmail model
     * work (for a web container this could be done in a filter before the thread is returned to the thread pool.)
     * 
     * @throws VoxmailException
     */
    public void releaseSession() throws VoxmailException {
        this.vmsm.releaseSession();
    }
    ...
    public void setBasePath(final String realPath) {
