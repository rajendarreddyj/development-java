...
public class InboxAction extends Action {
            ...
            if (cmd.startsWith("_")) { // we are on the secured side (retrieval, settings, etc)
                ...
                if (mailbox != null) {
                    ...
                    if (session.getAttribute("MailConnection") == null) {
                        this.getMailConnection(true, mailbox, request);
                    }

                    destination = "inbox"; // default (catches _continue)

                    if ("_getmail".equals(cmd)) {
                        this.sortMail(request);
                        destination = "inbox";
                    } else if ("_getmsg".equals(cmd)) {
                        skipRD = true;
                        this.streamAudio(request, response);
                        return null;
                    } else if ("_movemsg".equals(cmd)) { // set msg flag
                        String status = this.setFlag(request);
                        this.sortMail(request);
                        ...
                        // if (!isImap)
                        // {
                        this.getMailConnection(false, mailbox, request);
                        this.sortMail(request);
                        // }
                        destination = "inbox";
                    } else if ("_emptytrash".equals(cmd)) {
                        this.emptyTrash(request);
                        this.sortMail(request);
                        destination = "inbox";
                        ...
                        this.updatePIN(request, mailbox.getMailboxId());
                        destination = "menu";
                ...
                Mailbox mailbox = Voxmail.getInstance().getVoxmailService().retrieveMailbox(request.getParameter("contactid"));// getContact(request.getParameter("contactid"));
                                                                                                                               // //get
                                                                                                                               // contact
    ...
    protected void sortMail(final HttpServletRequest request) {
    ...
    protected void getMailConnection(final boolean isNewConnection, final Mailbox mailbox, final HttpServletRequest request) {
        HttpSession httpSession = request.getSession(false);
        MailConnection mail;
    ...
    protected void updatePIN(final HttpServletRequest request, final String mailboxId) {
        HttpSession httpSession = request.getSession(false);
    ...
    protected void streamAudio(final HttpServletRequest request, final HttpServletResponse response) {
        HttpSession httpSession = request.getSession(false);
