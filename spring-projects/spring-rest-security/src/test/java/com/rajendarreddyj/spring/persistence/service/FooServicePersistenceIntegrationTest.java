...
public class FooServicePersistenceIntegrationTest extends AbstractServicePersistenceIntegrationTest<Foo> {
    ...
    public final void whenEntityIsCreated_thenNoExceptions() {
        this.service.create(new Foo(randomAlphabetic(6)));
    }

    @Test(expected = DataIntegrityViolationException.class)
    public final void whenInvalidEntityIsCreated_thenDataException() {
        this.service.create(new Foo());
    }

    @Test(expected = DataIntegrityViolationException.class)
    public final void whenEntityWithLongNameIsCreated_thenDataException() {
        this.service.create(new Foo(randomAlphabetic(2048)));
    }
    ...
    public final void givenUsingCustomQuery_whenRetrievingEntity_thenFound() {
        ...
        this.service.create(new Foo(name));

        final Foo retrievedByName = this.service.retrieveByName(name);
