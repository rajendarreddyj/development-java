...
public class ActuatorMetricService implements IActuatorMetricService {
    ...
    private void exportMetrics() {
        final ArrayList<Integer> lastMinuteStatuses = this.initializeStatuses(this.statusList.size());

        for (final Metric<?> counterMetric : this.publicMetrics.metrics()) {
            this.updateMetrics(counterMetric, lastMinuteStatuses);
        }

        this.statusMetricsByMinute.add(lastMinuteStatuses);
    }
    ...
    private void updateMetrics(final Metric<?> counterMetric, final ArrayList<Integer> statusCount) {
        ...
        if (counterMetric.getName().contains("counter.status.")) {
            ...
            this.appendStatusIfNotExist(status, statusCount);
            index = this.statusList.indexOf(status);
