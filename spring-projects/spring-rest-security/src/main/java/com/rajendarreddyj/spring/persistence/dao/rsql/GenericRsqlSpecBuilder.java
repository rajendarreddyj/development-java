...
public class GenericRsqlSpecBuilder<T> {
    ...
    public Specifications<T> createSpecification(final LogicalNode logicalNode) {
        ...
        for (final Node node : logicalNode.getChildren()) {
            temp = this.createSpecification(node);
            if (temp != null) {
                specs.add(temp);
            }
        }
    ...
    public Specifications<T> createSpecification(final ComparisonNode comparisonNode) {
        final Specifications<T> result = Specifications
                .where(new GenericRsqlSpecification<T>(comparisonNode.getSelector(), comparisonNode.getOperator(), comparisonNode.getArguments()));
        return result;
    }
