...
/**
 * The DrawingPanel class provides a simple interface for drawing persistent images using a Graphics object. An internal
 * BufferedImage object is used to keep track of what has been drawn. A client of the class simply constructs a
 * DrawingPanel of a particular size and then draws on it with the Graphics object, setting the background color if they
 * so choose.
 * <p>
 * 
 ...
 * <p>
 * 
 * This version of DrawingPanel also saves animated GIFs, though this is kind of hit-and-miss because animated GIFs are
 * pretty sucky (256 color limit, large file size, etc).
 * 
 * This provided class helps make it easier to draw the assignment's graphics. A DrawingPanel object is a drawing
 * surface for painting 2D graphics.
 * 
...
public final class DrawingPanel extends Component implements ActionListener, ImageObserver, MouseMotionListener, Runnable, WindowListener {
    ...
    private static class ImageFrame {
        ...
        public ImageFrame(final Image image, final int delay) {
            this.image = image;
    ...
    private static boolean propertyIsTrue(final String name) {
        try {
    ...
    */
    ...
    private static boolean usingDrJava() {
        try {
            return (System.getProperty("drjava.debug.port") != null) || (System.getProperty("java.class.path").toLowerCase().indexOf("drjava") >= 0);
        } catch (SecurityException e) {
                    ...
                    public void run() {
                        try {
                            while (true) {
                                ...
                                if (((instances == 0) || DrawingPanel.this.shouldSave()) && !mainIsActive()) {
                                    try {
                                        System.exit(0);
    ...
    public void actionPerformed(final ActionEvent e) {
        if (e.getSource() instanceof Timer) {
            ...
            this.panel.repaint();
            if (this.shouldDiff() && (System.currentTimeMillis() > (this.createTime + (4 * DELAY)))) {
                ...
                try {
                    if (this.isAnimated()) {
                        this.saveAnimated(System.getProperty(SAVE_PROPERTY));
                    } else {
                        this.save(System.getProperty(SAVE_PROPERTY));
                    }
                } catch (IOException ioe) {
    ...
    public void addMouseListener(final MouseInputListener listener) {
    ...
    // returns the inner panel on which shapes are drawn
    ...
    // helper method to read an image and wait until it is loaded
    ...
    // Sets a border around the DrawingPanel area.
    ...
    // factor should be >= 1
    public void zoom(final int zoomFactor) {
    ...
    private void compareToURL() {
        ...
        try {
            ...
            if (filenames.isEmpty()) {
                JOptionPane.showMessageDialog(this.frame, "No valid web files found to compare against.", "Error: no web files found",
                ...
                if (filenames.size() == 1) {
                    ...
                    int choice = this.showOptionDialog(this.frame, "File to compare against?", "Choose File", filenames.toArray(new String[0]));
                    if (choice < 0) {
                        return;
                    }
                    // user chose a file; let's diff it
    ...
    private boolean isAnimated() {
        return this.animated || propertyIsTrue(ANIMATED_PROPERTY);
    }
    ...
    private boolean readyToClose() {
        ...
        */
        return ((instances == 0) || this.shouldSave()) && !mainIsActive();
    }

    private void replaceColor(final BufferedImage image, final Color oldColor, final Color newColor) {
    ...
    private String saveAsHelper(final String extension) {
    ...
    private class DiffImage extends JPanel implements ActionListener, ChangeListener {
        ...
        public DiffImage(final String file1, final String file2) throws IOException {
        ...
        public void drawImageFull(final Graphics2D g2, final BufferedImage image) {
        ...
        public void setImage1(final String filename) throws IOException {
        ...
        public void setImage2(final String filename) throws IOException {
            if (filename.startsWith("http")) {
    ...
    // ==============================================================================
    /**
     * Instances of this Gif89Frame subclass are constructed from RGB image info, either in the form of an Image object
     * or a pixel array.
     * <p>
     * There is an important restriction to note. It is only permissible to add DirectGif89Frame objects to a
     * Gif89Encoder constructed without an explicit color map. The GIF color table will be automatically generated from
     * pixel information.
     *
    ...
    class DirectGif89Frame extends Gif89Frame {
         ...
         * @param argb_pixels
         *            Array containing at least width*height pixels in the format returned by java.awt.Color.getRGB().
         */
        public DirectGif89Frame(final int width, final int height, final int argb_pixels[]) {
            this.theWidth = width;
    ...
    // ==============================================================================
    /**
     * This is the central class of a JDK 1.1 compatible GIF encoder that, AFAIK, supports more features of the extended
     * GIF spec than any other Java open source encoder. Some sections of the source are lifted or adapted from Jef
     * Poskanzer's <cite>Acme GifEncoder</cite> (so please see the <a href="../readme.txt">readme</a> containing his
     * notice), but much of it, including nearly all of the present class, is original code. My main motivation for
     * writing a new encoder was to support animated GIFs, but the package also adds support for embedded textual
     * comments.
     * <p>
     * There are still some limitations. For instance, animations are limited to a single global color table. But that
     * is usually what you want anyway, so as to avoid irregularities on some displays. (So this is not really a
     * limitation, but a "disciplinary feature" :) Another rather more serious restriction is that the total number of
     * RGB colors in a given input-batch mustn't exceed 256. Obviously, there is an opening here for someone who would
     * like to add a color-reducing preprocessor.
     * <p>
     * The encoder, though very usable in its present form, is at bottom only a partial implementation skewed toward my
     * own particular needs. Hence a couple of caveats are in order. (1) During development it was in the back of my
     * mind that an encoder object should be reusable - i.e., you should be able to make multiple calls to encode() on
     * the same object, with or without intervening frame additions or changes to options. But I haven't reviewed the
     * code with such usage in mind, much less tested it, so it's likely I overlooked something. (2) The encoder classes
     * aren't thread safe, so use caution in a context where access is shared by multiple threads. (Better yet, finish
     ...
     * Image objects created elsewhere in the program. Use of some of the most popular format options is also shown,
    ...
    class Gif89Encoder {
        ...
        // ----------------------------------------------------------------------------
        /**
         * Use this default constructor if you'll be adding multiple frames constructed from RGB data (i.e., AWT Image
         * objects or ARGB-pixel arrays).
         */
        public Gif89Encoder() {
        ...
        // ----------------------------------------------------------------------------
        /**
         * Like the default except that it also adds a single frame, for conveniently encoding a static GIF from an
         * image.
         *
         * @param static_image
         ...
         *
         * Use of this constructor imposes a couple of restrictions: (1) Frame objects can't be of type DirectGif89Frame
         * (2) Transparency, if desired, must be set explicitly.
         *
         * @param colors
         *            Array of color values; no more than 256 colors will be read, since that's the limit for a GIF.
         */
         ...
         *                See the addFrame() methods.
         */
        public Gif89Encoder(final Color[] colors, final int width, final int height, final byte ci_pixels[]) throws IOException {
            this(colors);
        ...
        // ----------------------------------------------------------------------------
        /**
         * Add a Gif89Frame frame to the end of the internal sequence. Note that there are restrictions on the
         * Gif89Frame type: if the encoder object was constructed with an explicit color table, an attempt to add a
         * DirectGif89Frame will throw an exception.
         *
         * @param gf
         *            An externally constructed Gif89Frame.
         * @exception IOException
         *                If Gif89Frame can't be accommodated. This could happen if either (1) the aggregate cross-frame
         *                RGB color count exceeds 256, or (2) the Gif89Frame subclass is incompatible with the present
         *                encoder object.
         */
        public void addFrame(final Gif89Frame gf) throws IOException {
        ...
        // ----------------------------------------------------------------------------
        /**
         * Convenience version of addFrame() that takes a Java Image, internally constructing the requisite
         * DirectGif89Frame.
         *
         * @param image
         *            Any Image object that supports pixel-grabbing.
         * @exception IOException
         *                If either (1) pixel-grabbing fails, (2) the aggregate cross-frame RGB color count exceeds 256,
         *                or (3) this encoder object was constructed with an explicit color table.
         */
         ...
         *            Array of color-index pixels no less than width * height in length.
         * @exception IOException
         *                Actually, in the present implementation, there aren't any unchecked exceptions that can be
         *                thrown when adding an IndexGif89Frame <i>per se</i>. But I might add some pedantic check
         *                later, to justify the generality :)
         */
        public void addFrame(final int width, final int height, final byte ci_pixels[]) throws IOException {
         ...
         *                encoder object.
         */
        public void insertFrame(final int index, final Gif89Frame gf) throws IOException {
            this.accommodateFrame(gf);
         ...
         *
         * @param index
         *            Index of the color that should be rendered as transparent, if any. A value of -1 turns off
         ...
         *
         * @param count
         *            Number of times to play sequence. Special value of 0 specifies indefinite looping. (Default: 1)
         */
        ...
        // ----------------------------------------------------------------------------
        /**
         * A convenience method for setting the "animation speed". It simply sets the delay parameter for each frame in
         * the sequence to the supplied value. Since this is actually frame-level rather than animation-level data, take
        ...
        // ----------------------------------------------------------------------------
        /**
         * After adding your frame(s) and setting your options, simply call this method to write the GIF to the passed
         * stream. Multiple calls are permissible if for some reason that is useful to your application. (The method
         * simply encodes the current state of the object with no thought to previous calls.)
         *
         * @param out
         ...
         *                If a write error is encountered.
         */
        public void startEncoding(final OutputStream out, final Image image, final int delay) throws IOException {
        ...
        public void continueEncoding(final OutputStream out, final Image image, final int delay) throws IOException {
            // write out the control and rendering data for each frame
            Gif89Frame gf = new DirectGif89Frame(image);
            this.accommodateFrame(gf);
        ...
        // ----------------------------------------------------------------------------
        private void accommodateFrame(final Gif89Frame gf) throws IOException {
        ...
        // ----------------------------------------------------------------------------
        private boolean isOk(final int frame_index) {
    ...
    class GifColorTable {
        ...
        void setTransparent(final int color_index) {
            this.transparentIndex = color_index;
        }

        // ----------------------------------------------------------------------------
        void processPixels(final Gif89Frame gf) throws IOException {
        ...
        // ----------------------------------------------------------------------------
        private void trackPixelUsage(final IndexGif89Frame igf) throws IOException {
    ...
    class ReverseColorMap {
        private class ColorRecord {
            ...
            ColorRecord(final int rgb, final int ipalette) {
                this.rgb = rgb;
    ...
    // ==============================================================================
    /**
     * First off, just to dispel any doubt, this class and its subclasses have nothing to do with GUI "frames" such as
     * java.awt.Frame. We merely use the term in its very common sense of a still picture in an animation sequence. It's
     * hoped that the restricted context will prevent any confusion.
     * <p>
     * An instance of this class is used in conjunction with a Gif89Encoder object to represent and encode a single
     * static image and its associated "control" data. A Gif89Frame doesn't know or care whether it is encoding one of
     * the many animation frames in a GIF movie, or the single bitmap in a "normal" GIF. (FYI, this design mirrors the
     * encoded GIF structure.)
     * <p>
     * Since Gif89Frame is an abstract class we don't instantiate it directly, but instead create instances of its
     * concrete subclasses, IndexGif89Frame and DirectGif89Frame. From the API standpoint, these subclasses differ only
     * in the sort of data their instances are constructed from. Most folks will probably work with DirectGif89Frame,
     * since it can be constructed from a java.awt.Image object, but the lower-level IndexGif89Frame class offers
     * advantages in specialized circumstances. (Of course, in routine situations you might not explicitly instantiate
     * any frames at all, instead letting Gif89Encoder's convenience methods do the honors.)
     * <p>
     * As far as the public API is concerned, objects in the Gif89Frame hierarchy interact with a Gif89Encoder only via
     * the latter's methods for adding and querying frames. (As a side note, you should know that while Gif89Encoder
     * objects are permanently modified by the addition of Gif89Frames, the reverse is NOT true. That is, even though
     * the ultimate encoding of a Gif89Frame may be affected by the context its parent encoder object provides, it
     * retains its original condition and can be reused in a different context.)
     * <p>
     * The core pixel-encoding code in this class was essentially lifted from Jef Poskanzer's well-known <cite>Acme
     * GifEncoder</cite>, so please see the <a href="../readme.txt">readme</a> containing his notice.
     *
     * @version 0.90 beta (15-Jul-2000)
     * @author J. M. G. Elliott (tep@jmge.net)
     * @see Gif89Encoder
    ...
    abstract class Gif89Frame {
         ...
         *
         * @param p
         *            Coordinates of the frame's upper left corner in the display space. (Default: The logical display's
         *            origin [0, 0])
        ...
        // ----------------------------------------------------------------------------
        /**
         * Setting this option determines (in a cooperative GIF-viewer) what will be done with this frame's display area
         * before the subsequent frame is displayed. For instance, a setting of DM_BGCOLOR can be used for erasure when
         * redrawing with displacement.
         *
         * @param code
         *            One of the four int constants of the Gif89Frame.DM_* series. (Default: DM_LEAVE)
         */
        ...
        // ----------------------------------------------------------------------------
        void encode(final OutputStream os, final boolean epluribus, final int color_depth, final int transparent_index) throws IOException {
        ...
        private void writeGraphicControlExtension(final OutputStream os, final boolean epluribus, final int itransparent) throws IOException {
            ...
            if ((transflag == 1) || epluribus) // using transparency or animating ?
            {
                ...
                putShort(this.csecsDelay, os); // delay field (2 bytes)
        ...
        private void writeImageDescriptor(final OutputStream os) throws IOException {
            ...
            putShort(this.thePosition.x, os);
            putShort(this.thePosition.y, os);
            putShort(this.theWidth, os);
            putShort(this.theHeight, os);
    ...
    // ==============================================================================
    /**
     * Instances of this Gif89Frame subclass are constructed from bitmaps in the form of color-index pixels, which
     * accords with a GIF's native palettized color model. The class is useful when complete control over a GIF's color
     * palette is desired. It is also much more efficient when one is using an algorithmic frame generator that isn't
     * interested in RGB values (such as a cellular automaton).
     * <p>
     * Objects of this class are normally added to a Gif89Encoder object that has been provided with an explicit color
     * table at construction. While you may also add them to "auto-map" encoders without an exception being thrown,
     * there obviously must be at least one DirectGif89Frame object in the sequence so that a color table may be
     * detected.
     *
     * @version 0.90 beta (15-Jul-2000)
     * @author J. M. G. Elliott (tep@jmge.net)
    ...
    class IndexGif89Frame extends Gif89Frame {
         ...
         */
        public IndexGif89Frame(final int width, final int height, final byte ci_pixels[]) {
            this.theWidth = width;
            this.theHeight = height;
    ...
    // ----------------------------------------------------------------------------
    /**
     * Write just the low bytes of a String. (This sucks, but the concept of an encoding seems inapplicable to a binary
     * file ID string. I would think flexibility is just what we don't want - but then again, maybe I'm slow.)
     */
    public static void putAscii(final String s, final OutputStream os) throws IOException {
     ...
     */
    public static void putShort(final int i16, final OutputStream os) throws IOException {
        os.write(i16 & 0xff);
        os.write((i16 >> 8) & 0xff);
    }
}